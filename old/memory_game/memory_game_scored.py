...
def main():
    ...
    # Scoring
    moves = 0
    optimal_moves = (BOARDWIDTH * BOARDHEIGHT) // 2
    ...
    startGameAnimation(mainBoard) # Gives you a sneak peak of the boxes
    ...
    while True:        
        ...
        # Display moves
        moves_surface = font.render(f'Moves: {moves}', True, (255, 255, 255))
        DISPLAYSURF.blit(moves_surface, dest=(0,0))
        ...
        if boxx != None and boxy != None:
            ...
            if not revealedBoxes[boxx][boxy]: # If box is not revealed
                drawHighlightBox(boxx, boxy) # Highlight the box
            if not revealedBoxes[boxx][boxy] and mouseClicked: # If you click on a concealed box
                revealBoxesAnimation(mainBoard, [(boxx, boxy)]) # Reveal box
                ...
                if firstSelection == None: # the current box was the first box clicked
                    ...
                    if icon1shape != icon2shape or icon1color != icon2color:
                        ...
                        if os.path.exists('highscore.pickle'):
                            highscore = pickle.load(open('highscore.pickle', 'rb'))
                            if highscore > moves:
                                highscore = moves
                        else:
                            highscore = moves
                        ...
                        moves_surface = font.render(f'Moves: {moves}', True, (255, 255, 255))
                        highscore_surface = font.render(f'Highscore: {highscore} moves', True, (255, 255, 255))
                        optimal_moves_surface = font.render(f'Optimal: {optimal_moves} moves', True, (255, 255, 255))
                        
                        gameWonAnimation(mainBoard, moves_surface, highscore_surface, optimal_moves_surface)
                        pygame.time.wait(2000)

                        # Reset the board
                        moves = 0
                        pickle.dump(highscore, open('highscore.pickle', 'wb'))
                        mainBoard = getRandomizedBoard()
                        revealedBoxes = generateRevealedBoxesData(False)
...
# Flash the background color if the player wins
def gameWonAnimation(board, s1, s2, s3):
    ...
    for i in range(13):
        ...
        drawBoard(board, coveredBoxes)
        DISPLAYSURF.blit(s1, dest=(0,0))
        DISPLAYSURF.blit(s2, dest=(0,25))
        DISPLAYSURF.blit(s3, dest=(0,50))
        pygame.display.update()
